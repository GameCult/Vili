...
function createIdunnRudpHealthPublisher(options) {
  ...
  return {
    ...
    sourceRuntimeId: options.sourceRuntimeId || "vili-daemon",
  };
}
...
async function publishIdunnRudpHealth(publisher, health) {
  ...
  try {
    ...
    const message = {
      ...
      messageId: `vili-health:${publisher.daemonId}:${observedAt.replace(/[:.]/g, "-")}`,
      document: {
...
async function receiveUntil(receiver, session, endpoint, predicate, timeoutMs, label) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (result.reply) {
        throw new Error("Vili RUDP health publisher received an unexpected reply-required packet.");
      }
  ...
  return {
    ...
    close() {
      ...
      while (waiters.length > 0) {
        const waiter = waiters.shift();
        clearTimeout(waiter.timer);
        const error = new Error("Vili RUDP health publisher closed.");
        error.code = "ECLOSED";
        waiter.reject(error);
      }
    },
